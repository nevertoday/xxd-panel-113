...
# XXD Panel 113｜Shell-fragment Pixel Collage
...
Observed limitations: some samples visibly exceed the original 200-module limit, retain oversized subjects or scenes, resemble regular mosaics more than rough shell flakes, or use overly muted colours. They show actual single-pass behaviour, not complete style compliance; no extra generations were made.
...
- `design-only`: the full canvas contains only Panel 113's designed translation; the photograph remains a non-visible reference.
...
Restart the agent session after installation, then invoke `$xxd-panel-113`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-113 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
...
/xxd-panel-113 ./photos --mode design-only --size auto,3:4 --text prompt --locale ja-JP
```

See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-113-prompt.en.md) or [Chinese](references/xxd-panel-113-prompt.zh-CN.md) runtime adapter.
