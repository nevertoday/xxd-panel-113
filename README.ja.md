...
以下のサンプルはそれぞれ異なる原画像を使い、Panel 113 が一枚ずつ独立した一回の生成で作成しました。AIメタデータは削除済みです。横長は左に実写、右にデザインを置く厳密な50:50、縦長は上に実写、下にデザインを置く厳密な50:50です。
...
確認された偏差：一部の作例は原文の上限200個を明らかに超え、被写体や残された場面が大きすぎ、粗い貝殻片より整ったモザイクに近く、色が暗く濁る場合もあります。単回生成の実際の挙動を示すもので、全条件に合格した標準作例ではありません。追加生成は行っていません。
